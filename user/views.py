    ...
    if request.method == "POST":
        ...
        if sifre == sifre2:
            if User.objects.filter(username = kullanici).exists():
                messages.error(request, 'Bu Kullanıcı ad zaten mevcut')
...
def create(request):
    form = ProfileForm()
    print("Profil Sayısı:", Profile.objects.filter(olusturan = request.user).count())
    if request.method == "POST":
    ...
    if request.method == "POST":
        ...
        if user is not None:
            if yeni1 == yeni2:
                ...
                kullanici.save()
                messages.success(request, 'Şifre değiştirildi Tekrardan giriş yapınız')
                return redirect('login')
            else:
                messages.error(request, 'Şifreler uyuşmuyor')
                return redirect('reset')
        else:
            messages.error(request, 'Girilen mevcut şifre yanlış')
            return redirect('reset')
